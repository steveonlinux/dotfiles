--[[
      ...
      * (c) 2013, Luca CPZ. 2019, Kirill Bugaev
...
-- return function that gets lines from end of file
function helpers.lines_from_reverse(path)
	local file, err_msg = io.open(path)
	if not file then
		local error_occur = 1
		return function()
			if error_occur == 1 then
				error_occur = 0
				return err_msg
			else
				return nil
			...
			while true do
				...
				repeat
					...
					if chunk_index < 0 then
						file:close()
						iterator = function()
							error('No more lines in file "'..path..'"', 3)
						end
						return
					end
...
function helpers.set_toggle_state(path, state)
	...
	local cmd = "echo -n " .. state_str .. " >" .. path .. " 2>/dev/null"
...
function helpers.rewrite_with_echo(path, str)
	local cmd = "echo -n " .. str .. " >" .. path .. " 2>/dev/null"
	helpers.async_with_shell(cmd, function() end)
end
...
-- deep table copy
function helpers.deep_table_copy(orig, copies)
	copies = copies or {}
	local orig_type = type(orig)
	local copy
	if orig_type == 'table' then
		if copies[orig] then
			...
			for orig_key, orig_value in next, orig, nil do
				copy[helpers.deep_table_copy(orig_key, copies)] = helpers.deep_table_copy(orig_value, copies)
			end
			copies[orig] = copy
			setmetatable(copy, helpers.deep_table_copy(getmetatable(orig), copies))
		end
	else -- number, string, boolean, etc
		copy = orig
	end
