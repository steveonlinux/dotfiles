...
local function factory(args)
	...
	net.followtag                   = args.followtag or false
	net.netstat_notification_preset = args.netstat_notification_preset or {}

	function net.hide_netstat_output()
		if not net.netstat_notification then return end
		naughty.destroy(net.netstat_notification)
		net.netstat_notification = nil
	end

	function net.show_netstat_output(scr)
		local cmd = "/usr/bin/netstat -tu"
		helpers.async(cmd, function(stdout, _)
			local notification_preset = net.netstat_notification_preset
			notification_preset.screen = net.followtag and awful.screen.focused() or scr or 1
			net.netstat_notification = naughty.notify {
	...
	function net.update()
		...
		for _, dev in ipairs(net.iface) do
			...
			if wifi_state == "on" and helpers.first_line(string.format("/sys/class/net/%s/uevent", dev)) == "DEVTYPE=wlan"
					and string.match(dev_now.carrier, "1") then
				dev_now.wifi   = true
				dev_now.signal = tonumber(string.match(helpers.lines_from("/proc/net/wireless")[3], "(%-%d+%.)")) or nil
			end

			if eth_state == "on" and helpers.first_line(string.format("/sys/class/net/%s/uevent", dev)) ~= "DEVTYPE=wlan"
					and string.match(dev_now.carrier, "1") then
		...
		settings(widget, net_now)
	end

	if showpopup == "on" then
		net.widget:connect_signal('mouse::enter', function() net.show_netstat_output(1) end)
		net.widget:connect_signal('mouse::leave', function() net.hide_netstat_output() end)
	end
