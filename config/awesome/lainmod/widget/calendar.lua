...
local markup       = require("lainmod.util.markup")
...
function calendar.show(t_out, inc_offset, scr)
	...
	calendar.notification_preset.screen = (calendar.followtag and awful.screen.focused()) or scr or 1
	...
	if current_month then -- today highlighted
		calendar.offset = 0
		-- calendar.icon = string.format(confdir .. "lain/icons/cal/white/%s.png", tonumber(os.date("%d")))
		-- calendar.icon = string.format("/home/user1/.config/awesome/lain/icons/cal/white/%s.png", tonumber(os.date("%d")))
	...
	helpers.async(f, function(ws)
		...
		local fg, bg = calendar.currentday.fg, calendar.currentday.bg
		calendar.notification_preset.text = ws:gsub("%c%[%d+[m]?%s?%d+%c%[%d+[m]?",
			markup.bold(markup.color(fg, bg, os.date("%e")))):gsub("[\n%s]*$", "")
...
function calendar.hover_on() calendar.show(0, 0, 1) end
function calendar.hover_off() calendar.hide() end
function calendar.prev() calendar.show(0, -1, 1) end
function calendar.next() calendar.show(0, 1, 1) end
...
local function factory(args)
	...
	calendar.currentday          = args.currentday or {}
	calendar.notification_preset = args.notification_preset or {}

	for _, widget in ipairs(calendar.attach_to) do calendar.attach(widget) end
end
