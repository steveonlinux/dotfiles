...
local function factory(args)
	local args      = args or {}
	local cs        = args.cs or base16.solarized_dark or {}
	local msg_tout  = args.msg_tout or 15
	...
	local bat_notification_critical_preset = {
		...
		timeout = msg_tout,
		fg      = cs.palette.barbg or "#000000",
		bg      = cs.palette.barbg_red or "#FFFFFF"
	}
	...
	local bat_notification_low_preset = {
		...
		text    = "Plug the cable!",
		timeout = msg_tout,
		fg      = cs.palette.barbg or "#202020",
		bg      = cs.palette.barbg_yellow or "#CDCDCD"
	}
	...
	local bat_notification_charged_preset = {
		...
		timeout = msg_tout,
		fg      = cs.palette.barbg or "#202020",
		bg      = cs.palette.barbg_green or "#CDCDCD"
	}
		...
		if notify == "on" then
			if bat_now.status == "Discharging" then
				if tonumber(bat_now.perc) <= n_perc[1] then
					for s in screen do
						local sid = s.index
						bat_notify_ids[sid] = naughty.notify( {
							preset = bat_notification_critical_preset,
							screen = sid,
							replaces_id = bat_notify_ids[sid]
						} ).id
					end
